...
class InstagramService {
     ...
     */
    async getLatestPostsAndPushDataToDb() {
        try {
            const profileMedia = await this.getProfileMedia();
            const mediaObjects = await this.createMediaArray(profileMedia);
    ...
    /**
     * Extracts relevant media URLs from the profile media and creates media object array.
     ...
     */
    async createMediaArray(profileMedia) {
        try {
            const mediaObjects = [];
            ...
            throw new Error("Error creating media array: " + error.message);
        }
    }
